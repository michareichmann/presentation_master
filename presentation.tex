	...
	\begin{center}
		\begin{minipage}{7cm}
			\begin{itemize}
				...
				\begin{itemize}
					\item \terminal{[-3, -19, -9, -193, -1, 96, -55, 58, 108, 164, 212, 93, -4, -6]}
				\end{itemize}
		...
		\begin{minipage}{3.5cm}
			\begin{figure}
				\includegraphics[width=3.2cm]{Pics/findDelay}
...
\begin{frame}
	...
	\begin{figure}
		\includegraphics[width=6cm]{Pics/clk_delay}
		\vspace*{-2pt}
...
\begin{frame}
	\begin{center}
		...
		\begin{minipage}{5.2cm}
			\begin{figure}
				\includegraphics[width=4.9cm]{Pics/address_lvls}
				\vspace*{-2pt}
...
% new frame =========================================
\begin{frame}
	\frametitle{Hit plane bits}
	\begin{figure}
		\includegraphics[height= 3cm]{Pics/hitplanebits}
	\end{figure}
	\begin{itemize}
		\item $2^{n}$ possibilities to hit \textit{n} planes
		\item information which configuration was hit how many times
		\item one hit in $0110$ means that there was one single event where ROC 1 \& 2 were hit and ROC 0 \& 3 where not hit
		\item allows to cut on special configurations (e.g. only two middle planes hit)
	\end{itemize}
\end{frame}
% new frame =========================================
\begin{frame}
	\frametitle{Tracking}
	\begin{itemize}
		\item software runs an alignment algorithm and puts the centre of the mask as centre of the x-y-plane
		\item all length units are in centimetres 
		\item tracks are only calculated if there is one cluster in every plane
		\item tracks as linear fits through every cluster centre
		\item maximum slope of the tracks $=\arctan \left(  \dfrac{size\ of\ a\ ROC}{max\ length\ telescope} \right)$ $=3.7^{\circ}$ in horizontal direction and $3.8^{\circ}$ in vertical direction 
	\end{itemize}
\end{frame}
%==============
